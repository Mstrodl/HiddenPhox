	...
	if(await isLoggingEnabled(ctx,msg) === true){
		...
		log.createMessage({embed:{
			...
			fields:[
				...
				{name:"Old Message",value:oldMsg.content.substring(0,128)+(oldMsg.content.length > 128 ? "..." : ""),inline:true},
				{name:"New Message",value:msg.content.substring(0,128)+(msg.content.length > 128 ? "..." : ""),inline:true},
...
let messageDelete = async function(msg,ctx){
	...
	if(await isLoggingEnabled(ctx,msg) === true){
		let fields = [
			{name:"ID",value:msg.id ? msg.id : "<no id given>",inline:true},
			{name:"Message",value:msg.content.substring(0,128)+(msg.content.length > 128 ? "..." : ""),inline:true},
	...
	if(await isLoggingEnabled(ctx,{channel:{guild:guild}}) === true){
		...
		log.createMessage({embed:{
			...
			fields:[
				...
				{name:"Created At",value:new Date(user.createdAt).toUTCString(),inline:true},
				{name:"Roles",value:user.roles ? user.roles.map(r=>`<@&${r}>`).join(" ") : "<uncached>",inline:true}
			],
			thumbnail:{
