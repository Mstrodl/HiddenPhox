...
let setStatus = function(ctx){
    ...
    status.name = status.name.replace("%scount%",ctx.bot.guilds.size).replace("%ucount%",ctx.bot.users.size);

    ctx.bot.editStatus("online",{type:status.type,name:status.name+" | hf!help"})
}
