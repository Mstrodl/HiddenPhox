...
function req(url) {
    return request
        ...
        .set({
            ...
            "Accept-Language": "en",
            "User-Agent":
                "Lynx/2.8.8rel.2 libwww-FM/2.14 SSL-MM/1.4.1 OpenSSL/1.0.2l",
            get "Set-Cookie"() {
                return setCookie;
            }
        })
...
function image(query, nsfw = false) {
    ...
    return req(
        `https://www.google.com/search?ie=ISO-8859-1&hl=en&source=hp&tbm=isch&gbv=1&gs_l=img&q=${query}&safe=${
            nsfw ? "disabled" : "active"
        }`
    )
...
async function search(query, nsfw) {
    ...
    // eslint-disable-next-line max-len
    const cardNode = xpath.select(
        `.//div[@id='rso']/div[@class='_NId']//div[contains(@class, 'vk_c') or @class='g mnr-c g-blk' or @class='kp-blk']`,
        root
    );
...
function parseGoogleCard(node) {
    ...
    if (calculator) {
        ...
        return `${query.data.trim()} ${answer.data.trim()}`.replace(
            "=",
            "equals"
        );
    }

    const unitConversions = xpath.select(
        `.//input[contains(@class, '_eif') and @value]`,
        node
    );
    if (unitConversions.length === 2) {
        const [firstValue, secondValue] = unitConversions.map(n =>
            n.getAttribute("value")
        );
        const unitSelector = `parent::div/select/option[@selected='1']/text()`;
        const [firstUnit, secondUnit] = unitConversions.map(
            n => xpath.select(unitSelector, n)[0]
        );
        return `${firstValue} ${firstUnit} = ${secondValue} ${secondUnit}`;
    }

    if (node.getAttribute("class").includes("currency")) {
        const currencySelectors = xpath.select(
            `.//div[@class='ccw_unit_selector_cnt']`,
            node
        );
        if (currencySelectors.length === 2) {
            const first = xpath.select(
                `.//div[@class='vk_sh vk_gy cursrc']/text()`,
                node
            )[0];
            const second = xpath.select(
                `.//div[@class='vk_ans vk_bk curtgt']/text()`,
                node
            )[0];
            return `${first} ${second}`;
        }
    ...
    const time = xpath.select(
        `.//div[contains(@class, 'vk_bk vk_ans')]/text()`,
        node
    )[0];
    if (time) {
        const location = xpath
            .select(".//span[not(@class)]/text()", node)[0]
            .data.trim();
